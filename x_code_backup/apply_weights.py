...
dump_file = open('C:\Users\zs633\Capstone\AUH\osm\duarouter\island.dump.xml', 'r')

weights_node_file = open('C:\Users\zs633\Capstone\AUH\osm\output\weights.nod.xml', 'w')
weights_edge_file = open('C:\Users\zs633\Capstone\AUH\osm\output\weights.edg.xml', 'w')
weights_dump_file = open('C:\Users\zs633\Capstone\AUH\osm\output\weights.dump.xml', 'w')
...
# for each node, average out weights from incoming edges
...
for edge in edges:
	...
	node = node_subtree.find('node[@id=\''+edge_to+'\']') # assumes that the node is present in node list
	node.set('weight_sum',str(float(node.get('weight_sum','0.0'))+float(edge.get('slowdown_ratio')))) # increase node's weight sum by edge's weight
	node.set('weight_num',str(float(node.get('weight_num','0.0'))+1.0)) # increment node's weight num

nodes = node_subtree.iter('node')
for node in nodes:
	node.set('avg_slowdown_ratio',str(float(node.get('weight_sum','0.0'))/float(node.get('weight_num','1.0')))) # average or 0.0 if no data
