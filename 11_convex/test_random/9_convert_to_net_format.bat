...
START /WAIT cmd.exe /K "%sumo_path%\netconvert -n %displaced_path%\convex.nod.xml -e %displaced_path%\convex.edg.xml -x %plain_path%\test.con.xml -i %plain_path%\test.tll.xml -o %output_net_path%\convex.net.xml -v --ignore-errors --ignore-errors.edge-type -X "never" -W & PAUSE & EXIT 0"
