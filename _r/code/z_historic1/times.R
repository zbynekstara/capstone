...
times = read.csv("/Users/Zbynda/Desktop/_0_SUMMER/R/data/times.csv",stringsAsFactors=FALSE)

# subsets times to exclude color-deficient subject

times_mean <- times[-c(33), c(ncol(times)-1)] # penultimate column
times_stdev <- times[-c(33), c(ncol(times))] # last column

times <- times[-c(33), -c(ncol(times),ncol(times)-1)]
#times # print
