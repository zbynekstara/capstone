...
gravity_node_file = open('./7_displaced/spherical.nod.xml', 'w')
gravity_edge_file = open('./7_displaced/spherical.edg.xml', 'w')
	...
	if is_junction: # if this node can move things
		...
		for other_node in other_nodes:
			...
			if nodes_distance > 0.0 and radius > 0.0 and nodes_distance <= radius: # must not be zero
				...
				new_dist = ((math.pow(radius,2.0)-math.pow(old_dist,2.0))/(math.pow(radius,2.0)+math.pow(old_dist,2.0)))*old_dist # x'-x = displacememnt
