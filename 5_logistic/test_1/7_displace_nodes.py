...
gravity_node_file = open('./7_displaced/logistic.nod.xml', 'w')
gravity_edge_file = open('./7_displaced/logistic.edg.xml', 'w')
	...
	if is_junction: # if this node can move things
		...
		for other_node in other_nodes:
			...
			if nodes_distance > 0.0: # must not be zero
				...
				new_dist = old_dist*(1.0-(1.0/(1.0+math.exp(-steepness*(old_dist-(radius/2.0)))))) # x'-x
