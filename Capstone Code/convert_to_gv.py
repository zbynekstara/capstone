...
graphviz_in_file = open('F:\Capstone\AUH\osm\output\graphviz.in.gv', 'w') # F:\Capstone\AUH\osm\output\graphviz.in.gv
...
for node in nodes:
	...
	gv_string += "\tnode_"+node.get('id').replace('-','a_dash_a').replace('#','a_hash_a')+" [pos=\""+node.get('x')+","+node.get('y')+"\"];\n"

edges = edge_subtree.iter('edge')
for edge in edges:
	# list edge connections
	# REVERSED WEIGHT!!	
	gv_string += "\tnode_"+edge.get('from').replace('-','a_dash_a').replace('#','a_hash_a')+" -- node_"+edge.get('to').replace('-','a_dash_a').replace('#','a_hash_a')+" [id=\"e_"+edge.get('id').replace('-','a_dash_a').replace('#','a_hash_a')+"\",weight=\"-"+edge.get('weight')+"\"];\n"

gv_string += "}"

# WRITE OUTPUT FILES
print "write output files"
graphviz_in_file.write(gv_string)

# CLOSE FILES
print "close files"
weights_node_file.close()
weights_edge_file.close()
graphviz_in_file.close()