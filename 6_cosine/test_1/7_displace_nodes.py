...
gravity_node_file = open('./7_displaced/cosine.nod.xml', 'w')
gravity_edge_file = open('./7_displaced/cosine.edg.xml', 'w')
...
# DISPLACE NODES BASED ON THE COSINE EFFECT OF MOST CONGESTED NODES
	...
	if is_junction: # if this node can move things
		...
		for other_node in other_nodes:
			...
			if nodes_distance > 0.0 and radius > 0.0 and nodes_distance <= radius: # must not be zero
				...
				new_dist = old_dist*(math.cos((math.pi/2)*(old_dist/radius))) # x'-x
