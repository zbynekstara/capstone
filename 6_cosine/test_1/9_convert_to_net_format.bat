...
START /WAIT cmd.exe /K "%sumo_path%\netconvert -n %displaced_path%\cosine.nod.xml -e %displaced_path%\cosine.edg.xml -x %plain_path%\test.con.xml -i %plain_path%\test.tll.xml -o %output_net_path%\cosine.net.xml -v --ignore-errors --ignore-errors.edge-type -X "never" -W & PAUSE & EXIT 0"
