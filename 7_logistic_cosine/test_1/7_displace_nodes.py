...
gravity_node_file = open('./7_displaced/logistic_cosine.nod.xml', 'w')
gravity_edge_file = open('./7_displaced/logistic_cosine.edg.xml', 'w')
	...
	if is_junction: # if this node can move things
		...
		for other_node in other_nodes:
			...
			if nodes_distance > 0.0 and radius > 0.0 and nodes_distance <= radius: # must not be zero
				...
				#VERSION 1
				cosine_portion = 0
				if nodes_distance <= radius:
					cosine_portion = math.cos((math.pi/2.0)*(old_dist/radius))
				logistic_portion = 1.0-(1.0/(1.0+math.exp(-steepness*(old_dist-(radius/2)))))
				new_dist = old_dist*(cosine_portion+(logistic_portion*(1-cosine_portion)))
				...
				if nodes_distance <= radius:
					cosine_portion = math.cos((math.pi/2.0)*(old_dist/radius))
				logistic_portion = 1.0-(1.0/(1.0+math.exp(-steepness*(old_dist-(radius/2)))))
				new_dist = old_dist*cosine_portion*logistic_portion # x'-x
				"""
				...
				if nodes_distance <= radius:
					cosine_portion = math.cos((math.pi/2.0)*(old_dist/radius))
				logistic_portion = 1.0-(1.0/(1.0+math.exp(-steepness*(old_dist-(radius/2)))))
				new_dist = old_dist*((cosine_portion+logistic_portion)/2.0) # x'-x
				"""
